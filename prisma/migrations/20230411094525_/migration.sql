/*
  ...
  - You are about to alter the column `type` on the `Admin_Type` table. The data in that column could be lost. The data in that column will be cast from `VarChar(50)` to `Enum(EnumId(7))`.
  ...
  - A unique constraint covering the columns `[type]` on the table `Admin_Type` will be added. If there are existing duplicate values, this will fail.
...
-- AlterTable
ALTER TABLE `Admin_Type` MODIFY `type` ENUM('admin', 'manager', 'staff') NOT NULL DEFAULT 'staff',
